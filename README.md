...
Room media is paced in 64 KiB chunks through one shared adaptive bandwidth budget. The budget starts conservatively, decreases when viewer RTT or queueing delay rises, and grows gradually while the connection remains stable. All viewers share it, preventing several simultaneous streams from independently saturating the host's upload. libmpv uses a bounded 20-second/64 MiB forward cache with hysteresis instead of unrestricted read-ahead.
